...
!!! warning "Cuidado con la accesibilidad"
...
Esta página se centra en el flujo normal y en los valores de `display` que lo gobiernan (`block`, `inline`, `inline-block`, `none`, `contents`, `table`); para el detalle completo de cómo funcionan estos dos modelos de layout, sus propiedades y sus casos de uso, consulta las páginas dedicadas: [Flexbox](flexbox.md) y [CSS Grid](grid.md).
